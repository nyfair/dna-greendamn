...
fn count(base: usize) -> u32 {
    match mem::read_u32(base + GOBJECTS_RVA + OFF_NUM_ELEMENTS) {
...
fn chunks_ptr(base: usize) -> usize {
    ...
    match mem::read_u32(array + OFF_NUM_CHUNKS) {
        Some(n) if n > 0 && n <= 0x1000 => {}
        _ => return 0,
    }
    mem::read_u64(array + OFF_OBJECTS).unwrap_or(0) as usize
}
...
fn chunk_at(chunks: usize, chunk_idx: u32) -> usize {
    ...
    mem::read_u64(chunks + chunk_idx as usize * 8).unwrap_or(0) as usize
}
...
fn item_at(chunk: usize, index: u32) -> usize {
    ...
    mem::read_u64(chunk + (index % ELEMENTS_PER_CHUNK) as usize * FUOBJECT_ITEM).unwrap_or(0)
        as usize
}
...
fn describe(module: &Module, pool: usize, obj: usize) -> Option<Described> {
    ...
    if unsafe { mem::read_raw(obj, blk.as_mut_ptr(), blk.len()) } != blk.len() {
        return None;
    }
    ...
    let class_tag = if CLASS_OK[idx].load(Ordering::Relaxed) == want_a {
        ...
        let class_id = match mem::read_u32(class + names::OFF_NAME) {
            Some(v) => v,
            None => return None,
        };
        let mut buf = [0u8; 64];
        let len = names::read_fname(pool, class_id, &mut buf);
        if len == 0 {
            return None;
        }
        let tag = if names::matches(&buf[..len], CAM_CLASS) {
            TAG_CAM
        } else if names::matches(&buf[..len], FUNC_CLASS) {
            TAG_FUNC
        } else {
            TAG_OTHER
        };
...
fn name_tag(pool: usize, name_id: u32) -> u32 {
    ...
    let mut buf = [0u8; 64];
    let len = names::read_fname(pool, name_id, &mut buf);
    let tag = if len > 0 && names::matches(&buf[..len], DISABLE_NAME) {
        TAG_DISABLE
    } else {
        TAG_OTHER
    };
...
unsafe fn call_disable(module: &Module, cam: usize, func: usize) -> bool {
    ...
    let pe = match mem::read_u64(vt + PROCESSEVENT_IDX * 8) {
        Some(v) if v != 0 => v as usize,
        _ => return false,
    };
...
fn log_camera(module: &Module, pool: usize, cam: usize) {
    if CAM_EVER_LOGGED.swap(1, Ordering::Relaxed) == 0 {
        ...
        b.push_str(" '");
        names::push_class_name(&mut b, module, pool, cam);
        b.push_byte(b'\'');
        log_buf(&b);
    }
}
